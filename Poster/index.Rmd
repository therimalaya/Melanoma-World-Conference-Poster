...
```{r, include=FALSE}
source("~/Documents/Papers/Analysis/scripts/00-functions.R")
data <- readRDS("~/Documents/Papers/Analysis/data/extended.rds")
data[, Gender := factor(fifelse(Gender == "Male", "Men", "Women"), levels = c("Women", "Men"))] %>% 
  data.table::setnames("Gender", "Sex")
...
tbl$table_body[2, grep("^stat", names(tbl$table_body))] <- as.list(purrr::pluck(missing_breslow, 'label'))
...
tbl$table_body[4, 1] <- list("T1 (≥1mm)")
tbl$table_body[5, 1] <- list("T2 (>1-2mm)")
tbl$table_body[6, 1] <- list("T3 (>2-4mm)")
tbl$table_body[7, 1] <- list("T4 (>4mm)")
